...
// ==================== PENAMBAHAN BLE-MAODV MULTI-METRIC SYSTEM ====================

/**
 * @brief Metrics for BLE-MAODV path evaluation
 */
struct BLEMetrics {
    double residualEnergy;    // 0.0 - 1.0 (normalized)
    double rssiValue;         // in dBm (typically -100 to -30)
    double stabilityScore;    // 0.0 - 1.0
    uint32_t hopCount;
    Time lastUpdated;
    
    // Hanya deklarasi - implementasi di .cc
    BLEMetrics();
    
};

/**
 * @brief Weight factors for adaptive metric calculation
 */
...
/**
 * @brief Network context for adaptive weighting
 */
struct NetworkContext {
    double nodeDensity;        // 0.0 - 1.0
    double mobilityLevel;      // 0.0 - 1.0  
    double energyCriticality;  // 0.0 - 1.0
    double trafficCriticality; // 0.0 - 1.0
    
    NetworkContext()
        : nodeDensity(0.5),
          mobilityLevel(0.5),
          energyCriticality(0.5),
          trafficCriticality(0.5)
    {
    }
};

/**
 * @brief Adaptive weighting calculator for BLE-MAODV
 */
class AdaptiveWeightCalculator {
public:
    AdaptiveWeightCalculator();
    WeightFactors CalculateWeights(const NetworkContext& context) const;
    void UpdateNetworkContext(NetworkContext& context, 
                             uint32_t neighborCount,
                             double avgEnergy,
                             double mobilityIndicator) const;
    
private:
    static constexpr double HIGH_DENSITY_THRESHOLD = 0.7;
    static constexpr double HIGH_MOBILITY_THRESHOLD = 0.6;
    static constexpr double HIGH_ENERGY_THRESHOLD = 0.3;
    
    WeightFactors m_highDensityWeights;
    WeightFactors m_highMobilityWeights;
    WeightFactors m_energyCriticalWeights;
    WeightFactors m_defaultWeights;
};

// ==================== END BLE-MAODV MULTI-METRIC SYSTEM ====================
...
/**
 * @ingroup aodv  
 * @brief Multipath Routing table entry for BLE-MAODV
 */
class MultipathRouteEntry 
{
public:
  MultipathRouteEntry();
  MultipathRouteEntry(Ipv4Address destination);
  ...
  struct PathInfo {
    ...
    PathInfo();
    double CalculateCompositeScore(const WeightFactors& weights) const;
    void UpdateStabilityScore(bool successfulTransmission);
  };
  
  // Method declarations - PERBAIKAN: Gunakan MultipathRouteEntry::PathInfo
  void AddPath(Ipv4Address nextHop, uint32_t hopCount, Time lifetime);
  void AddPath(const MultipathRouteEntry::PathInfo& pathInfo);  // PERBAIKAN DI SINI
  void RemovePath(Ipv4Address nextHop);
  PathInfo GetBestPath();
  std::vector<PathInfo> GetAllPaths();
  bool HasValidPath();
...
class RoutingTable 
{
  public:
    ...
    // ========== PENAMBAHAN MULTIPATH METHODS ==========
    /**
     * @brief Add multipath route entry
     */
    bool AddMultipathRoute(Ipv4Address dst, Ipv4Address nextHop, uint32_t hopCount, Time lifetime);

    /**
     * @brief Get best multipath route for destination
     */
    bool GetBestMultipathRoute(Ipv4Address dst, MultipathRouteEntry::PathInfo& pathInfo);  // PERBAIKAN

    /**
     * @brief Get all multipath routes for destination
     */
    std::vector<MultipathRouteEntry::PathInfo> GetAllMultipathRoutes(Ipv4Address dst);  // PERBAIKAN

    /**
     * @brief Check if multipath route exists for destination
     */
    bool HasMultipathRoute(Ipv4Address dst);

    /**
     * @brief Remove specific multipath route
     */
    bool RemoveMultipathRoute(Ipv4Address dst, Ipv4Address nextHop);

    /**
     * @brief Purge expired multipath routes
     */
    void PurgeMultipathRoutes();
  ...
  private:
    ...
    /// Multipath routing table - TAMBAHAN BARU
    std::map<Ipv4Address, MultipathRouteEntry> m_multipathTable;
